...
func (r *BytebaseReconciler) syncIngress(ctx context.Context, bb *bbv1.Bytebase, hostname string, token string) error {
	var err error
...
func (r *BytebaseReconciler) syncNginxIngress(ctx context.Context, bb *bbv1.Bytebase, host string) error {
	...
	if _, err := controllerutil.CreateOrUpdate(ctx, r.Client, ingress, func() error {
		cookie, err := r.getClientCookie()
		if err != nil {
			return err
		}
		snippet := fmt.Sprintf("proxy_set_header Cookie: \"%s\"", cookie)
		expectIngress := createNginxIngress(bb, host, snippet)
...
func (r *BytebaseReconciler) getClientCookie() (string, error) {
	...
	s := fmt.Sprintf("access-token=%s; refresh-token=%s; user=%s", accessToken, refreshToken, user)
	return s, nil
}
